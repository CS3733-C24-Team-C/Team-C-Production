...
class AStarPathfindingStrategy implements IPathfindingStrategy {
    async findPath(startNodeId: string, endNodeId: string, graph: Graph): Promise<string[]> {
        ...
        const heuristic = (nodeIdA: string, nodeIdB: string): number => {
            ...
            const horizontalDistance = Math.sqrt(Math.pow(nodeA.xcoord - nodeB.xcoord, 2) + Math.pow(nodeA.ycoord - nodeB.ycoord, 2));

            const verticalCostMultiplier = verticalDistance > 0 ? (nodeA.nodeType === 'ELEV' || nodeB.nodeType === 'ELEV' ? 1 : 2) : 0;
        ...
        while (openSet.length > 0) {
            ...
            currentNodeId.forEach(neighbor => {
                ...
                if (!neighborNode) {
                    neighborNode = { nodeId: neighbor.endNode, gCost: Infinity, hCost: heuristic(neighbor.endNode, endNodeId), fCost: Infinity };
